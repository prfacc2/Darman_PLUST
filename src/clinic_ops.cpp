...
// Parse a ticket's servicesJson into a list of {code, name} pairs.
...
std::string SvReport_Json(const SvReportQuery& q){
    ...
    // Build a map: serviceCode → {serviceName, rows:[...]}
    // Each row: {seq, name, nid, insBase, insSupp, insCombo, paid}
    struct ReportRow {
        int seq;
    ...
    std::map<std::wstring,int> blockIdx; // serviceCode → index in blocks

    // Helper: compare Jalali dates (YYYY/MM/DD) as strings works lexicographically.
    ...
    // Shift name mapping: stored shift names may be صبح/عصر/شب or بعد از ظهر.
    auto shiftMatches=[&](const std::wstring& tShift)->bool{
        ...
        // fallback: compare against shiftDisplayName
    ...
    for(const auto& t : tickets){
        if(t.status==L"cancelled") continue;
        // Date filter: use jdate (Jalali YYYY/MM/DD), fallback to apptDate.
        ...
        for(const auto& s : svcs){
            ...
            // Use the most recent name in case it changed.
            if(!s.name.empty()) blk.serviceName=s.name;

            // Build the insurance combo cell.
    ...
    o+="],";
    // Footer: current shift + time + date.
    SYSTEMTIME st=iranNow();
