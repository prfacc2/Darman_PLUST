...
void  setFlatButtonStyle(HWND btn, int style); // in-place style swap (E2 tool toggle)
...
// Resolve a shift index to its display name (via defs, fallback to shiftName).
std::wstring shiftDisplayName(int idx);
// Resolve a shift index to its start/end minutes.
void shiftDefHours(int idx, int& startMin, int& endMin);
// Match a stored shift-name string to a shift index (handles «عصر» / «بعد از ظهر»).
...
struct ServiceDef {
    ...
    std::wstring name;      // نام کامل (full name — appears on receipt + report title)
    std::wstring category;  // دسته/گروه (kept on disk for back-compat; UI retired v1.74)
    std::wstring dept;      // بخش (section id or name)
    ...
    // name; multiplier is a raw decimal string ("1.5"). The six tariff columns
    // hold the current + «new» Rial prices for آزاد / دولتی / بیمه tariffs.
