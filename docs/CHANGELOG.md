...
## 1.91.0 — 2026-08-25 — بازطراحی کامل UI روی «سیستم طراحی» واحد + ساختار ضدخرابکاری + صندوق بخش‌ها

> این نسخه دو کار بزرگ انجام می‌دهد: (۱) ظاهر برنامه را روی **یک** سیستم طراحی
> واحد بازنویسی می‌کند، و (۲) ساختار پروژه را جوری عوض می‌کند که آپدیت‌های بعدی
> نتوانند بی‌صدا صفحه‌ها و آیتم‌های بخش‌های دیگر را خراب کنند.

### Added — ساختار و قانون (ریشهٔ مشکل «آپدیت ۹ یکی از صفحه‌های مدیریت را پاک کرد»)
- **`AGENTS.md`** (+ `CLAUDE.md` به آن لینک است) — قانون کار روی مخزن برای هر مدل
  هوش مصنوعی یا توسعه‌دهنده: افزودنی بودن تغییرات، دست‌نزدن به محاسبات مالی و
  قالب فایل‌های داده، قوانین لایه‌بندی CSS، قرارداد دو-موتوره، و قوانین کارایی.
- **`docs/ARCHITECTURE.md`** — نقشهٔ راه: هر بخش کدام فایل است، مالک هر لایه
  کیست، شش سطح صفحهٔ تعبیه‌شده، ثبت منابع RCDATA، مدل داده و تله‌های آن.
- **`docs/DESIGN_SYSTEM.md`** — تنها مرجع رنگ و ظاهر برنامه: پالت کامل
  (بوم/سطح/جوهر/حاشیه + هفت رنگ معنایی با چهار پله)، دستور سایه، شیشهٔ تقلیدی،
  دستور فیلد، دستور هدر بخش با **نیم‌خط کوتاه**، مقیاس تایپوگرافی، و جدول پورت
  پالت به `theme.cpp`.
- **`scripts/check_ui_contract.py`** — قرارداد UI حالا **اجرایی** است و
  `build.sh` پیش از کامپایل آن را اجرا می‌کند. بررسی می‌کند: ثبت منابع در
  `app.rc` / inliner / `index.html` هم‌خوان است؛ هر سطح تمام `id`های موردنیاز
  جاوااسکریپتش را دارد؛ هیچ کامپوننتی بدون استایل نمانده؛ لایه‌بندی تم
  دوباره روی هم انبار نشده؛ CSS روی **هر دو موتور** رندر می‌شود؛ جاوااسکریپت
  ES5 مانده؛ و رنگ‌های ردشده (بنفش/صورتی/کِرِم) برنگشته‌اند. **بیلد قرمز = تحویل ناتمام.**

### Changed — بازسازی لایهٔ CSS (پایان جنگ آبشاری)
- `assets/admission/admission.css` از ۵۹۷۸ خط به **لایهٔ ساختار** کوتاه شد. حدود
  ۴۴۰۰ خط شامل **شانزده** لایهٔ متوالی «پولیش نهایی» (v1.65 → v1.90) که هر کدام
  روی قبلی رنگ می‌پاشید حذف شد — همان چیزی که باعث می‌شد نتیجهٔ هر تغییر رنگ
  غیرقابل پیش‌بینی شود و ظاهر برنامه سه بار رد شود.
- به جایش **لایهٔ مرتب تم** آمد؛ هر لایه یک مالک انحصاری دارد
  (`css/core.css`, `css/surface-dash.css`, `css/surface-tools.css`,
  `css/surface-admission.css`, `css/surface-cashier.css` — منابع RCDATA ۴۲۰..۴۲۴).
- **بلوک `SURFACE GATING`** در لایهٔ ساختار: تصمیم اینکه کدام زیردرخت روی صفحه
  است، بین‌سطحی و ساختاری است، پس یک‌جا و فقط همان‌جا نگه داشته می‌شود.
- `backdrop-filter` واقعی حذف شد: فقط روی WebView2 کار می‌کرد و روی
  MSHTML/Trident بی‌اثر بود؛ حالا شیشه با گرادیان‌های لایه‌ای تقلید می‌شود تا
  هر دو موتور یک‌شکل باشند.

### Changed — صفحهٔ ورود (نیتیو C++)
- پردهٔ روی تصویر پس‌زمینه از `bg`@۵۸ به **`#E4ECF8`@۸۶** رفت: صفحه دیگر تیره
  نیست و با عکس تداخل ندارد.
- پنل قهرمان **سفیدگرا** شد (`#FFFFFF → #EEF3FB`، حاشیهٔ `#B9C8DF`).
- **سینی آیکون‌ها هم‌عرض دقیق پنل پشت سرش** شد (`panel.left`/`panel.right`)، با
  پرِ روشن `#F9FBFE → #E8EFF9`، سایهٔ خنثی و **هالهٔ نرم بیرونی** `#A6C6F0`.
- خط جداکنندهٔ نازک بین دو حساب: `#C6D2E4`@۱۴۰.
- **glow و صفحهٔ رنگی زیر آیکون حساب‌ها حذف شد** — فقط خود آیکون با سایهٔ خنثی
  می‌ماند (خواستهٔ صریح کارفرما). نشان‌ها: پرسنل آبی نیلی، مدیریت فیروزه‌ای.
- نقاشی همچنان dirty-rect است؛ هیچ تایمر، انیمیشن یا حلقهٔ سایهٔ تازه‌ای اضافه نشد.

### Changed — هدر و فوتر نیتیو
- نوار هدر روشن‌تر شد: `#F2F6FD → #E9EFF9 → #D8E2F1` با موکِش `#C6D2E4` و
  زیرخط نیلی ۲px.
- **ساعت ۲۲px پررنگ `#16253C`** (بود ۱۹px) و **تاریخ جلالی ۱۴px `#3B4C69`**
  (بود ۱۲px) — با فونت‌های اختصاصی `g_fClock`/`g_fDate` تا اندازهٔ فونت‌های
  مشترک برنامه دست‌نخورده بماند؛ ناحیهٔ ساعت به `S(300)` پهن شد تا تاریخ بلند
  بریده نشود.
- ربان هدر از نیلی→بنفش به **نیلی→لاجوردی** رفت (بنفش ممنوع شد).
- فوتر حالا فونتش را صریح انتخاب می‌کند و به ارث‌بری از تاریخ وابسته نیست.

### Changed — پالت نیتیو (`theme.cpp`)
- تم روشن کامل روی جدول §۱۱ سیستم طراحی پورت شد.
- **`g_infoAccent` از بنفش `#7C56E4` به لاجوردی `#0E7FC0`** رفت (در تم تیره هم).

### Changed — داشبورد پرسنل
- بوم طراحی‌شده (گرادیان سه‌ایستگاهی + دو هالهٔ کم‌شفافیت + بافت مورب ۱px).
- هدر **نوار شیشه‌ای شناور** با ربان هویت؛ جستجو کوتاه، وسط‌چین، قرصی با حاشیهٔ
  واضح — نه inset و نه concave.
- اپ-آیکون‌ها: squircle با سایهٔ واقعی، **بدون هاله/صفحهٔ رنگی**؛
  پذیرش بیمار (نیلی) راست، تب جدید (فیروزه‌ای) چپ. کارتابل پیام فقط در هدر.

### Changed — ابزارها و جستجوی قبض
- **پالت کِرِم/رسی v1.90 حذف شد** (کارفرما آن را رد کرد) و جایش بومِ **فولادی
  سرد** آمد که با تم برنامه هم‌خوان است.
- تفاوت با داشبورد حالا **ساختاری** است نه رنگی: هدر تخت و چسبیده به لبه،
  ریل دسته‌بندی راست‌چین، و کاشی‌ها به **ردیف‌های فهرستی** تبدیل شدند.
- همهٔ زیرعنوان‌های توضیحی حذف/پنهان ماندند.
- راهنمای رنگ قبض از رنگ‌های دلبخواه به پالت رسمی رفت؛ «بدهکار» از بنفش به
  خاکستری-آبی.

### Changed — پذیرش بیمار
- **ریشهٔ باگ رنگ‌های بی‌ربط رفع شد:** کلاس‌های `.ins-card`/`.doc-card`/`.perf-card`
  ده قانون CSS داشتند اما **روی هیچ عنصری در HTML نبودند**، پس رنگ‌بندی موردنظر
  هرگز اعمال نمی‌شد و tint‌های موقعیتی قدیمی از زیر بیرون می‌زد — همان
  «بنفش روی هدر سبز». کلاس‌ها به مارک‌آپ اضافه شدند.
- قانون **یک رنگ برای هر پنل**: هدر، نیم‌خط، چیپ آیکون و جوهر عنوان همه از یک
  رنگ می‌آیند. مشخصات بیمار/خدمات/عملیات پذیرش نیلی، بیمه و انجام‌دهنده
  لاجوردی، پزشک معالج فیروزه‌ای، صورتحساب و مبلغ نهایی زمردی، چاپ و وضعیت خاکستری-آبی.
- **نیم‌خط کوتاه ۲۶×۳px** زیر عنوان هر بخش؛ خط کاملِ پررنگ بالای کارت‌ها حذف شد.
- فیلدها **صفحهٔ سفید تخت با حاشیهٔ واضح** شدند؛ حالت concave/فرورفته حذف شد.
- ناحیهٔ بردردار **خدمات** حالا از هر چهار طرف ۱۰px فاصله دارد و کشسان است —
  دیگر فضای مردهٔ پایین ندارد.
- **صورتحساب** زمردی شد (کِرِم/کهربایی قبلی حذف)؛ **مبلغ نهایی** ۳۴px/۹۰۰ و
  بزرگ‌ترین عدد صفحه.
- برچسب‌ها و مقادیر پرکنتراست شدند. **محاسبات قبض دست‌نخورده است.**

### Added / Changed / Fixed — صندوق «بخش‌ها»
- **Added:** `cashier.page` حالا `sections[]` و `totals` هم برمی‌گرداند — خلاصهٔ
  هر **بخش** (بیمار، صندوق‌نرفته و مبلغش، صندوق‌شده و مبلغش) در همان یک پیمایش
  ۲۴ساعته و با همان محدودیت دسترسی. نام بخش‌ها از `sections.dat` می‌آید و در
  «مدیریت» تعریف می‌شود — هیچ نامی در کد hard-code نشده.
- **Added:** جدول **یک ردیف برای هر بخش** با هدر و جمع چسبان؛ با ۲ بخش و ۲۲ بخش
  یک‌شکل می‌ماند و ارتفاع صفحه تغییر نمی‌کند.
- **Changed:** نوار زبانه‌ها (شامل «صندوق نرفته‌ها») **از هدر بیرون آمد و
  مستقیماً بالای جدول** نشست.
- **Changed:** پنل شیفت طراحی‌شده: چیپ وضعیت، تاریخ و ساعت **شروع و پایان**،
  متصدی و بخش، و درآمد شیفت. برای شیفت بسته `lastShift` اضافه شد.
- **Fixed:** دکمهٔ **شروع شیفت** وقتی شیفت باز است غیرفعال می‌شود و **پایان شیفت**
  فعال؛ و برعکس. دکمهٔ غیرفعال دیگر دیالوگ تأیید را هم باز نمی‌کند.
- **Fixed:** اگر شیفتی باز نباشد، پرداخت ثبت می‌شد ولی درآمد **بی‌صدا گم**
  می‌شد؛ حالا پرداخت انجام می‌شود و هشدار «شیفتی باز نیست» نمایش داده می‌شود.
- قالب `cashier_tickets.dat` / `shifts.dat` / `sections.dat` تغییر نکرد.

### Fixed — رگرسیون‌های خودِ همین بازسازی (که با قرارداد UI گرفته شدند)
- بلوک **surface gating** که در کوتاه‌سازی `admission.css` از دست رفته بود
  برگشت: بدون آن، سطوح ابزارها/صف/قبض فرم پذیرش را زیر خودشان نشان می‌دادند.
- کامپوننت‌هایی که فقط در لایه‌های حذف‌شده تعریف شده بودند (خانوادهٔ ردیف‌های
  **صف**، خلاصهٔ صف، دیالوگ تأیید مشترک و چند مورد دیگر) بازیابی شدند و حالا
  بررسی «پوشش کامپوننت» جلوی تکرار این اتفاق را می‌گیرد.

### Removed
- `scripts/test_admission_assets.py` — این تست به پالت قدیمی گره خورده بود و از
  قبل هم روی سه assertion کهنه قرمز بود. بررسی‌های معتبرش به
  `scripts/check_ui_contract.py` منتقل شد و بقیه با معماری جدید بی‌معنا بودند.
