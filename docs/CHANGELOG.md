...
## 2.08.1 — 2026-09-02 — رفع باگ‌های v2.08 + بهبود UI جوابدهی آزمایشگاه

### رفع باگ
- **باگ دیالوگ تنظیمات فونت/بزرگنمایی**: `fzPaint` به bitmap خارج از صفحه نقاشی می‌کرد اما هرگز آن را با `BitBlt` روی صفحه کپی نمی‌کرد — فقط تکست‌باکس جستجو ظاهر می‌شد. اضافه شد `BitBlt` و اصلاح ایجاد bitmap از `dc0` (`font_settings.cpp`).
- **باگ جستجوی داشبورد و ابزارها**: جستجو شامل `data-name` پنهان بود که کلمات کلیدی آیتم‌های دیگر را داشت (مثلاً «پ» روی «لیست سیاه بیماران» مطابقت داشت چون data-name شامل «مسدودسازی پذیرش» بود). اصلاح: فقط برچسب قابل‌مشاهده (`dash-appname`/`tools-tile-name`) جستجو شود (`admission.js`).

### بهبود UI جوابدهی آزمایشگاه
- **تم روشن برای باکس‌های شیشه‌ای**: باکس سوال و باکس تک‌بیمار در تم سفید حالا روشن (سفید/آبی روشن) هستند نه تیره. تم تیره/نئون رنگ‌های تیره خود را حفظ می‌کند (`admission.css`).
- **فونت Vazir**: باکس‌های آزمایش حالا از `Vazirmatn` استفاده می‌کنند.
- **اعداد انگلیسی**: پیشوند ماه و پلیس‌هولدرها به اعداد انگلیسی تغییر یافت. ورودی‌های شماره قبض و بارکد `dir="ltr"` شدند (`index.html`, `admission.js`).
- **تمرکز پیش‌فرض**: باکس تک‌بیمار باز می‌شود، پیشوند ماه به انگلیسی نوشته می‌شود و تمرکز روی ورودی بعد از `-` قرار می‌گیرد (`admission.js`).
- **ناوبری کیبورد**: باکس سوال از کلیدهای جهت‌نما (بالا/پایین) برای حرکت بین گزینه‌ها، Enter برای انتخاب و Escape برای بستن پشتیبانی می‌کند. کادر نازک انتخاب (`lab-q-focused`) اضافه شد (`admission.js`, `admission.css`).

### بهبود خوانایی پذیرش
- **افزایش ۲px سایز فونت لیبل‌ها**: ۱۴px → ۱۵px برای لیبل اصلی، ۱۲.۵–۱۳px → ۱۳.۵–۱۴px برای سایر متن‌های کوچک. هنوز رزولوشن-آوار است (`admission.css`).

## 2.08.0 — 2026-09-01 — ریسپانسیو پذیرش، تنظیمات فونت/بزرگنمایی، جستجوی قبض آپدیت، جوابدهی آزمایشگاه

### ریسپانسیو و خوانایی صفحه پذیرش
- **رفع قطع آیتم‌ها**: دو `@media` که `overflow:hidden` را روی ≤۱۱۸۰px اجبار می‌کردند (و باعث محو شدن خوشه چاپ و نوار خدمات می‌شد) به `overflow-y:auto` تغییر یافت (`admission.css:928,2095`).
- **بزرگنمایی خودکار (autoFitZoom)**: اندازه محتوای طبیعی در ۱۰۰٪ اندازه‌گیری و بزرگترین زومی که در مانیتور جا می‌شود محاسبه می‌شود؛ Ctrl+0 حالا auto-fit می‌کند نه ۸۰٪ ثابت (`admission.js:autoFitZoom`).
- **بزرگنمایی پایدار**: زوم و فونت از `settings.ini` خوانده می‌شوند (قبلاً ۸۰٪ هاردکد بود) (`web_admission_api.inc:init`).
- **افزایش سایز لیبل‌ها**: ۱۱–۱۳px به ۱۲.۵–۱۴px؛ رنگ `#2D3E54` (خاکستری) به `#1B2A45` (سرمه‌ای تیره) برای تم سفید (`admission.css`).
- **به‌روزرسانی روی تغییر اندازه**: `window.resize` با debounce دوباره auto-fit می‌کند (مگر اینکه کاربر زوم دستی داده باشد).

### تنظیمات فونت و بزرگنمایی (جدید)
- **ورودی در تنظیمات C++**: ردیف «تنظیمات فونت و بزرگنمایی» در هدر تنظیمات اضافه شد (`user_settings.cpp:ROW_FONT_ZOOM`).
- **دیالوگ مستقل** (`font_settings.cpp`): لیست قابل‌جستجوی فونت‌های نصب‌شده (EnumFontFamiliesExW)، استپر بزرگنمایی (خودکار/۵۰–۲۰۰٪)، سایز فونت (۸–۳۲px)، وزن (نازک/کلفت)، پیش‌نمایش زنده.
- **ذخیره و اعمال زنده**: چهار کلید در `settings.ini` ذخیره و از طریق `reception.settings` به همه تب‌های پذیرش پوش می‌شود.

### جستجوی قبض (آپدیت)
- **ارتفاع بیشتر**: جدول نتایج حالا ارتفاع کامل ستون را می‌گیرد (رفع `min-height` ثابت).
- **جدول جزئیات پایین**: با تک‌کلیک روی هر قبض، خدمات آن در جدول جداگانه‌ای پایین صفحه نمایش داده می‌شود (ستون‌ها: کد خدمت، کد ملی خدمت، نام خدمت، تعداد، قیمت کل، بیمه پایه، بیمه مکمل، پرداختی، قیمت کل).
- **ستون‌های قابل تغییر اندازه**: کاربر می‌تواند عرض هر ستون را با درگ تغییر دهد.
- **دکمه چاپ قبض**: زیر دکمه جستجو اضافه شد (`#rcPrintSide`).
- **ذخیره کد ملی سلامت**: `ServiceLine.natCode` به ساختار داده اضافه و در `cashier_tickets.dat` ذخیره می‌شود.

### جوابدهی بیمار (آزمایشگاه — جدید)
- **جایگزینی «تب جدید»**: دکمه داشبورد و دکمه هدر فریم «تب جدید» با «جوابدهی بیمار» جایگزین شد (`index.html`, `main.cpp`).
- **باکس سوال**: با کلیک، باکس شیشه‌ای (glassmorphism) با ۷ گزینه (فقط «جوابدهی تک بیمار» فعال) ظاهر می‌شود.
- **باکس جوابدهی تک بیمار**: ورودی شماره قبض (پیش‌فرض ماه جاری) یا بارکد + دکمه‌های تأیید/جستجو/انصراف.
- **تب جدید «جوابدهی تک بیمار»**: اطلاعات بیمار (شماره، بارکد، پزشک، نام، سن، جنسیت، پذیرش، زمان آماده شدن، تکمیل، تعداد چاپ، وضعیت پرداخت با رنگ سبز/قرمز، تلفن، ایمیل) + جدول نتایج آزمایش (۹ ستون) + پنل اطلاعات/امضای سمت راست + نوار بالای صفحه (بیمار بعدی/قبلی، سایز کاغذ، پیش‌نمایش، چاپ کامل) + سایدبار لیست بیماران (به‌روزرسانی، چاپ گروهی، فیلتر).
- **`Ops_IsLab`**: پیش‌دیکشن جدید برای تشخیص بخش آزمایشگاه (`clinic_ops.cpp`).
- **افزودن به صفحه ابزارها**: دکمه «جوابدهی بیمار» به صفحه ابزارها هم اضافه شد.

---

## 2.07.1 — 2026-08-30 — سیاه‌وسفیدِ خالص طرح‌ها + رفع گالری خالی

### Fixed — ۳۰ طرح آماده و گالری
- **گالری طراح چاپ خالی بود.** `assets/designer/templates.js` در v2.07.0 به یک فهرست نام خالی (`items: []`) کاهش یافته بود و همهٔ کارت‌های گالری صفحهٔ سفید نشان می‌دادند. حالا این فایل دوباره آینهٔ دقیق آهنگساز C++ است: همان ۱۳ بلوک اجباری، همان جدول ۳۰ ردیفی، ۶۶ آیتم در هر طرح، یک بارکد و یک جدول خدمات.
- **طرح‌ها رنگی بودند ولی چاپگر درمانگاه مونوکروم است.** همهٔ طرح‌های آماده و طرح دیفالت اکنون سیاه‌وسفیدِ خالص‌اند — فقط `#000000`، بدون باند رنگی سربرگ و بدون کادر رنگی. تأکید فقط از طریق ضخامت قلم (بولد)، ضخامت خط (۰.۳۰/۰.۴۵/۰.۶۰ میلی‌متر) و کادر است، نه رنگ.
- مهاجرت جدید `tpl_migration_2_07_1` هر ۳۱ طرح ذخیره‌شده را برای نصب‌هایی که قبلاً مهاجرت ۲.۰۷ را اجرا کرده‌اند بازمی‌سازد (شناسه‌ها و اتصال ماشین حفظ می‌شود؛ طرح‌های کاربر دست‌نخورده می‌مانند).
- تست رگرسیون هر دو قاعده را اجباری می‌کند: بدون ثابت رنگ در آهنگساز/آینه، `textColor` فقط مشکی، و `templates.js` باید به ۳۱ طرح کامل با ۱۳ بلوک اجرا شود (۱۷۹ نامتناقض، تست منفی ×۲).

## 2.07.0 — 2026-08-30 — چاپ و پذیرش: «ارتباط با چاپگر»، موتور چاپ برداری، ۳۰ طرح آماده، بیمهٔ تکمیلی، زیربخش پذیرش

### Added — «ارتباط با چاپگر» (تنظیمات)
- دیالوگ جدید `AzPrinterLink` در `src/printer.cpp` با جستجوی چاپگر (نرمال‌سازی فارسی `uikit::NormalizeFa`، زیررشته)، لیست چاپگرها، نشان «پیش‌فرض ویندوز»، سوییچ «پیرو پیش‌فرض ویندوز» (کلید `printer_follow_windows_default`)، «تست اتصال» و «بازخوانی». ذخیره از `printerRequestGate` با عنوان «ارتباط با چاپگر» و تغییر «تغییر چاپگر برنامه» می‌گذرد.
- ردیف «ارتباط با چاپگر» در پنل تنظیمات زنده (`src/user_settings.cpp`، `ROW_PRINTER_LINK`)؛ مسیر `PrinterLink_Open` در `src/app.h`.
- `docs/WORKORDER_2_07_MANIFEST.md` — مانیفست قطعی همهٔ نشانی‌های این نسخه.

### Fixed — موتور چاپ (`src/printer.cpp`)
- **شرح تخصص چاپ نمی‌شد.** `{specialty}` حالا زنجیرهٔ `r.specialty` → مخزن پزشکان با `doctorCode` → مخزن پزشکان با نام دقیق `treatingDoctor` → خالی را طی می‌کند (بدون جعل). پذیرش وب هم `specialty` و `medicalId` را هنگام ثبت ذخیره می‌کند (`src/web_admission_api.inc`).
- **کد بارکد ۲ بار چاپ می‌شد.** مقدار بارکد فقط به‌صورت HRI زیر نمودار چاپ می‌شود؛ محافظ `pdBarcodeValueAlreadyRendered` (`src/print_services_policy.h`) هر آیتم متنی بعدی با همان مقدار را رد می‌کند — برای طرح‌های قدیمی کاربر هم برقرار است.
- **سن بیمار چاپ نمی‌شد.** «سن» به‌عنوان خانهٔ مستقل در طرح دیفالت و هر ۳۰ طرح حضور دارد؛ نام بیمار (`{P-Name}`) هرگز نمی‌شکند — تک‌خطی با کاهش ۰.۵ پوینتی تا ۷۰٪ و سپس سه‌نقطه.
- **کیفیت و رنگ چاپ.** متن بالای روشنایی ۰.۶۲ تیره می‌شود (هیچ خاکستری روشن‌تر از #555555)، خطوط حداقل ۰.۳۰ میلی‌متر و هرگز روشن‌تر از #333333، روی همهٔ متن‌های چاپ از جمله HRI بارکد و سلول‌های جدول.
- **فضای خالی زیر جدول خدمات.** بند کشسان «بلوک_پایانی»: هر آنچه زیر جدول خدمات است به اندازهٔ (ارتفاع طبیعی − ارتفاع تألیف) پایین می‌رود، محدود به ناحیهٔ چاپ‌پذیر؛ فاصلهٔ تنفسی ۴.۰ میلی‌متر روی کاغذ و ۲.۵ روی رول حرارتی؛ صفحه‌بندی فقط در مرز واقعی صفحه.
- بازآرایی واکنش‌گرا: `reflowDesign()` در مسیر چاپ برای تغییر اندازهٔ کاغذ (طرح‌های قدیمی baseW==0 بدون مقیاس پذیرفته می‌شوند).
- توکن‌های جدید `{certno}`، `{receipttitle}`، `{clinicname}` + نام‌های خام؛ `{clinicaddr}/{clinicphone}` از رکورد → `clinic_addr` → `clinic_address` برطرف می‌شوند تا نام پیکربندی‌شدهٔ مدیریت روی رسید بیاید.

### Changed — ۳۰ طرح آماده (`src/print_designer_templates.inc`)
- هر ۳۰ طرح با یک آهنگساز قطعی `بساز_طرح(طرح_پارامتر)` ساخته می‌شوند که همیشه ۱۳ بلوک اجباری را به‌ترتیب تولید می‌کند؛ کدهای T01–T30 ثابت. T01 طرح جایگزین است.
- جدول خدمات در همهٔ طرح‌ها دقیقاً «نام خدمت | تعداد | شرح خدمت» با عرض تطبیقی شرح (جمع‌شدن به ۰.۱۸ وقتی همه خالی‌اند، رشد تا ۰.۵۲).
- حذف از همهٔ چیدمان‌ها: ردیف متنی بارکد تکراری، «شماره پرونده» و «شماره سابقه» (توکن‌ها و رندرکننده‌ها برای طرح‌های کاربر باقی‌اند — RULE 6).
- تأیید دودِ `pdVerifyBuiltinTemplates` در اسکرین `print_designer`: ۱۳ بلوک، دقیقاً یک بارکد، یک جدول ۳ ستونه، صفر «شماره سابقه»، حداکثر یک `{receiptbarcode}`.
- `TB1 «برچسب بارکد زیربخش»` — برچسب فقط-بارکد با مجموعهٔ بلوک کوتاه (§7.5).

### Changed — صفحهٔ پذیرش
- لیبل «نوع بیمه» → «بیمه تکمیلی»؛ سلکت از فهرست بیمه‌های تکمیلی فعال با فعل جدید «بیمه_تکمیلی_فهرست» پر می‌شود و انتخاب با «بیمه_تکمیلی_انتخاب» `suppIdx`/`suppPercent` را از `Supp_Percent` می‌گذارد — یک مسیر برای فرم، محاسبهٔ صورتحساب و `{supp_percent}/{supppay}`.
- ورودی «شماره شناسنامه» (`#certNo`) برای توکن `{certno}`.
- ارتفاع کمبو «پزشک معالج» با باکس جستجوی «انجام‌دهنده» همسان شد (۳۴px، فقط `body.surface-adm`).
- باکس جستجوی «خدمات پذیرش» از راست +۲۸px عریض شد (۲۰۵→۲۳۳px).
- کلیک روی هدر «خدمات پذیرش» (فقط پس‌زمینه/متن/آیکون) باکس جستجو را فوکوس می‌کند؛ کلیک روی بدنه، ردیف‌ها، اسکرول‌بار یا دکمه‌های هدر بی‌اثر است.

### Changed — هدر حساب پرسنلی
- خط دوم هدر «نوع حساب» را نشان می‌دهد (پذیرش / مدیریت / کارآموز / پذیرش آزمایشگاه)؛ اولویت: سمت پرسنلی → نقش پروفایل کارمند → برچسب بخش (`Sections_AccountRoleLabel`) → fallback سطح دسترسی. نام همیشه نام کامل است؛ یوزرنیم فقط آخرین راه‌حل و با `logError` ثبت می‌شود.

### Added — زیربخش پذیرش (`src/sections.h/.cpp`, `src/clinic_ops.cpp`)
- ستون اختیاری دوازدهم `recept_sub` با تیک «زیربخش پذیرش» در CRM (فقط زیربخش‌ها، پیش‌فرض خاموش)؛ `Sections_IsReceptionSub` بدون ارث‌بری از والد.
- زیربخش پذیرش بدون پوز نمی‌تواند پرداخت را کامل کند: کنترل‌های پرداخت غیرفعال (حذف نمی‌شوند) + اعلان «پرداخت در بخش پذیرش انجام می‌شود»؛ «ثبت قبض» بلیت را unpaid به صندوق پذیرش اصلی می‌فرستد.
- درآمد به صندوق پذیرش اصلی ثبت می‌شود؛ روی رسید «پذیرش» نام کامل اپراتور زیربخش، «صندوق‌دار» نام کامل صندوق‌دار پذیرش اصلی و «ش.ص» شمارهٔ شیفت پذیرش اصلی است (هیچ‌وقت یوزرنیم).
- چاپ زیربخش فقط `TB1` است — مسیریابی در نقطهٔ ورودی چاپ، نه UI؛ ورودی‌های طراح/تنظیمات چاپ برای این scope غیرفعال با توضیح.

## 2.06.0 — 2026-08-29 — پذیرش بدون اتوسلکت، کارتابل دوطرفه، جدول خدمات ۳ ستونه، بدون ۳ نقطه، رول ۱۸ سانتی، تنظیمات چاپ، Ctrl+Tab سراسری

### Fixed — پذیرش بیمار (پزشک معالج / انجام دهنده)
- **اتوسلکت خاموش شد.** تایپ کد/نام در «کد نظام پزشکی» دیگر هیچ پزشکی را خودکار انتخاب نمی‌کند؛ فقط لیست پیشنهاد زیر فیلد باز می‌شود و پذیرشگر یا کلیک می‌کند یا هیچی (`autoSelectDoctor` حذف شد از هر دو مسیر تایپ زنده و Enter؛ `assets/admission/admission.js`).
- **لیست «انجام دهنده» خالی بود — ریشه:** `init` پاکت کامل `{"rows":[…]}` را می‌فرستاد ولی JS آن را آرایه فرض می‌کرد؛ `state.performers` شیء می‌شد، حلقه پر کردن اجرا نمی‌شد و کمبو تا ابد «— انتخاب —» می‌ماند. حالا هر دو شکل باز می‌شوند + زنجیره fallback تا هیچ‌وقت خالی نماند (`fillPerformers`).
- «انجام دهنده» حالا **پزشک و پرستار** را نشان می‌دهد (`docType` برچسب پرستار)، کد آن **کد نظام پزشکی یا کد پزشک** است و همان فیلد جستجوی زنده + لیست پیشنهاد (`#perfResults`) را دارد.
- فرم نیتیو پذیرش هم دیگر اولین پزشک/انجام‌دهنده را از پیش انتخاب نمی‌کند — ردیف «— انتخاب —» اضافه شد (`src/reception.cpp`).
- پزشکان غیرفعال از جستجو حذف شدند (`src/web_admission_api.inc`).

### Fixed — کارتابل پیام (چت)
- **پیام‌های خود کاربر در چت نمایش داده نمی‌شد.** `loadMessages` فقط ردیف‌های «گیرنده من» را برمی‌گرداند؛ پیام‌های ارسالی خود کاربر هرگز برنمی‌گشتند. حالا `portal.messages.list` و `crm.messages.list` هر دو جهت گفتگو را برمی‌گردانند با پرچم قطعی `mine` (بدون تطبیق رشته‌ای نام کامل/نام کاربری که هرگز match نمی‌شد).
- ردیف‌های ممیزی خودکار («کاربر … پیام را مشاهده کرد») از گفتگو حذف شدند.
- ایندکس ردیف‌های دریافتی با قرارداد جدیدترین-اول لایه داده هماهنگ شد (pin/seen/delete روی ردیف درست عمل می‌کنند)؛ ردیف‌های ارسالی شناسه منفی یکتا می‌گیرند و pin/delete/save روی آن‌ها غیرفعال است.
- تایل پیام ارسالی خود کاربر با استایل و برچسب «ارسال شما» متمایز می‌شود (`admission.css`, `crm.css`).
- پیشوند «↩» از پاسخ‌ها حذف شد تا متن تمیز نمایش داده شود.

### Changed — جدول خدمات چاپ (درخواست صریح کاربر)
- **جدول خدمات فقط ۳ ستون دارد: نام خدمت / تعداد / شرح خدمت.** هیچ چیز دیگز (مبلغ کل، مبلغ واحد، تخفیف، سهم بیمه/بیمار، کد خدمت، ردیف، نوع خدمت، پرداختی، قیمت کل، سهم پایه، سهم مکمل) در جدول نیست (`svcModelJson` در `src/print_designer_templates.inc` + همان سیاست در `assets/designer/templates.js`).
- همه فیلدهای پولی/شناسنامه‌ای زیر جدول در فوتر آمده‌اند: **پرداختی، قیمت کل، سهم پایه، سهم مکمل، تخفیف از، تخفیف، تخفیف نقد، POS، کد رهگیری نسخه الکترونیک، شماره معرفی نامه، پذیرش، صندوق دار، ش.ص، تاریخ و ساعت** (`emitSamenFinance`, `emitSamenRef`, `emitSamenFooter`).
- مهاجرت `tpl_migration_2_06`: هر ۳۱ طرح builtin بازسازی **و** طرح‌های ذخیره‌شده کاربر با مدل قدیمی به مدل ۳ ستونه مهاجرت می‌کنند.

### Fixed — چاپ بدون ۳ نقطه و بدون تداخل
- **هیچ متنی در چاپ تبدیل به «…» نمی‌شود.** همه `DT_END_ELLIPSIS` از مسیرهای چاپ حذف شدند و با کوچک‌شدن فونت تا حد جا شدن + شکستن خط (wrap) جایگزین شدند: سلول‌های جدول خدمات (`pdPaintServiceLogicalRow`)، سرستون‌ها (`pdDrawServices`)، جدول‌های ثابت (`pdDrawTable`)، قبض کلاسیک (`src/billing.cpp`) و دوقلوی پیش‌نمایش آن‌ها در `src/manage.inc`.
- عامل px-per-pt واقعی DC به حلقه shrink-to-fit پاس می‌شود تا روی پرینترهای ۳۰۰–۶۰۰ DPI اندازه درست بماند.
- پیش‌نمایش طراح چاپ هم مثل چاپ wrap می‌کند (`assets/designer/designer.css` — قاعده `text-overflow:ellipsis` حذف شد) تا WYSIWYG برقرار بماند.

### Added — تفکیک خدمات: رول ۱۸ سانتی + تنظیمات چاپ + UI
- **رول پیش‌فرض از ۸ سانتی به ۱۸۰mm (۱۸ سانتی) بزرگ شد** — همه چیز جا می‌شود (`svrPaperDims`، `@page`، گزینه «رول حرارتی ۱۸۰ میلی‌متر»).
- پیش‌نمایش روی صفحه با عرض واقعی کاغذ انتخابی رندر می‌شود (`data-svr-paper`) و **رنگ پس‌زمینه برگه وسط از پس‌زمینه اطراف متمایز است** + سایه کاغذ.
- **دکمه «تنظیمات چاپ»** جدید: ساختار جدول (کامل/خطوط افقی/بدون خطوط)، چیدمان ستون‌ها، سایز کاغذ، حاشیه، اندازه فونت، تک‌رنگ، تکرار سرستون، ردیف فشرده — همه با **پیش‌نمایش WYSIWYG زنده** که دقیقاً همان CSS چاپ را استفاده می‌کند.

### Fixed — Ctrl+Tab سراسری
- Ctrl+Tab حالا از **هر جایی** کار می‌کند: فریم اصلی، داخل HTML (WebView2/MSHTML)، نوار تب، هدر، حساب پرسنل. گیت قدیمی `GetAncestor(...)==g_hFrame` حذف شد و به جای آن هر keydown از پروسه خود برنامه بررسی می‌شود؛ شتاب‌دهنده WebView2 دیگر Ctrl+Tab را نمی‌بلعد و به فریم پس می‌دهد (`src/main.cpp`, `src/web_admission_host.inc`, `assets/admission/admission.js`).

### Fixed — ابزار بیلد
- `shot.sh` فاقد `src/accounting.cpp` در SRCS بود (لینک دیباگ شکست می‌خورد) — اضافه شد.

---

## 2.05.0 — 2026-08-28 — راست‌کلیک، پزشک، حساب کاربری، صندوق، چاپ، کارتابل

### Fixed
- راست‌کلیک HTML فقط کپی / برش / چسباندن.
- کد نظام پزشکی دیگر با ایندکس لیست عوض نمی‌شود (۴۵۶ → ۹).
- تعریف حساب کاربری همه پرسنل را یکجا لود نمی‌کند؛ JSON بخش‌ها معتبر شد.
- تیک صندوق بخش و زیربخش همزمان خطا می‌دهد.
- صندوق تب‌ها را از `cashier_tab` می‌سازد و زیربخش را هم می‌بیند.
- چاپ: کاغذ A4/A5 دیگر به اندازهٔ طرح A4 کوچک نمی‌شود.
- کارتابل: ارسال پاسخ بدون دیالوگ صندوق؛ حباب سبز واتساپی.
- تفکیک خدمات: عرض ۸۰mm، تاریخ انتهای هر خدمت، وضعیت پرداخت، صفحه‌بندی ۳/۵/۱۰.
- Ctrl+Tab بین تب‌های پذیرش (راست به چپ، حلقه).

---

## 2.04.0 — 2026-08-28 — پذیرش نسخه ۲.۰۰، فیلتر تفکیک خدمات، داشبورد سریع

### Fixed — پذیرش بیمار
- چیدمان و زوم ۸۰٪ روی `#appBody` مثل نسخه ۲.۰۰ برگشت؛ بقیه تب‌ها دست نخورده‌اند.

### Added — تفکیک خدمات
- دکمه «فیلترها» در هدر؛ پنل تنظیم از/تا تاریخ، شیفت، بخش، زیربخش، پزشک، خدمت، پرداخت، کاغذ و بقیه.
- پیش‌فرض: همه + تاریخ همان روز (۲۴ ساعت).

### Fixed — داشبورد پرسنل
- دیگر پشت «هیچ پذیرشی باز نیست» گیر نمی‌کند؛ تب قبل از ساخت WebView ثبت می‌شود و محیط WebView2 قبل از ورود گرم است.

---

## 2.03.0 — 2026-08-28 — گزارش تفکیک خدمات، پذیرش بدون زوم، طرح‌های من

### Fixed — تفکیک خدمات
- با دادهٔ خالی دیگر روی «در حال آماده‌سازی گزارش» گیر نمی‌کند.
- نوار درصد واقعی (۰ تا ۱۰۰) هنگام خواندن قبض‌ها و جدول‌بندی.
- Bridge `.catch` به‌جای `.fail`؛ watchdog تا گیرکردن ابدی نماند.

### Fixed — پذیرش بیمار
- HTML ظرف C++ را با `height:100%` پر می‌کند، نه با زوم CSS. زوم پیش‌فرض ۱۰۰٪ ماند.

### Fixed — تنظیمات چاپ
- لیست راست فقط «پیش‌فرض» + طرح‌های خود کاربر است؛ ۳۰ طرح آماده در این لیست نیست.
- پیش‌نمایش دیگر JSON بارکد (`code128` / `hri`) را به‌صورت متن چاپ نمی‌کند.

### Fixed — طراحی چاپ HTML
- ارتفاع جدول خدمات مثل پیش‌نمایش C++ است (بدون کشیدن جدول به کل باکس).

---

## 2.02.0 — 2026-08-28 — پذیرش تمام‌ظرف، تب تفکیک خدمات، رنگ دکمه گزارش

### Fixed — صفحه پذیرش بیمار
- WebView2 دیگر روی اندازهٔ لحظهٔ ساخت قفل نمی‌ماند: `WebAdmission_Resize` + `put_Bounds` با ظرف C++ همگام است.
- زوم پیش‌فرض از ۸۰٪ به ۱۰۰٪ آمد تا صفحه وسط نماند و از کنار/پایین فاصله نداشته باشد.
- بزرگ‌نمایی هر تب جداست؛ تب‌های بازنشده تغییر نمی‌کنند.

### Fixed — تفکیک خدمات
- کلیک روی کاشی دیگر «تب جدید» خالی باز نمی‌کند؛ تب HTML «تفکیک خدمات» باز می‌شود.
- هستهٔ تب‌ها با `tabKindSurface()` یک‌جا نگاشت می‌شود تا سطح HTML بعدی از allow-list جا نماند.
- نام از «به تفکیک خدمات» به «تفکیک خدمات» تغییر کرد.

### Fixed — رنگ دکمه
- آیکون گزارش در تم روشن/تیره/نئون دیگر سفید نیست (گرادیان بنفش / فیروزه‌ای نئون).

### Files
- `src/reception.cpp`, `src/web_admission_host.inc`, `src/web_admission_api.inc`, `src/app.h`, `src/app.rc`
- `assets/admission/admission.js`, `admission.css`, `index.html`
- `update/version.txt`, `README.md`, `docs/CHANGELOG.md`

---

## 2.00.0 — 2026-08-27 — همگام‌سازی زنده + قالب پیش‌فرض ثامن + چاپ پس از پرداخت

### Fixed
- شروع/پایان شیفت بلافاصله در همه تب‌ها دیده می‌شود؛ ثبت قبض وضعیت زنده را می‌خواند.
- هشدار دقیق: «برای پذیرش بیمار ابتدا باید شروع شیفت را بزنید»
- صندوق، درآمد، جستجوی قبض بدون رفرش به‌روز می‌شوند.
- صفحه سیاه هنگام باز شدن تب HTML حذف شد.
- جستجوی قبض: انتخاب ردیف، رنگ‌ها، ستون‌های کامل، بدون چک‌باکس خالی.
- ابزار انتخاب / ابزار دست در طراح چاپ.
- چاپ فقط پس از پرداخت کامل.

### Added
- قالب پیش‌فرض حرارتی ثامن‌الائمه + ۳۰ طرح آماده متفاوت.

---

## 1.99.0 — 2026-08-27 — قبض، درآمد شیفت، طراح چاپ per-machine، نئون

### Fixed — پذیرش و صندوق
- هر پذیرش یک قبض می‌سازد و در جستجوی قبض دیده می‌شود (۳۰ مورد اخیر بدون اجبار جستجو).
- درآمد شیفت پذیرشگر با پرداخت (و پوز) افزایش می‌یابد. بن‌بست درآمد شیفت رفع شد.
- بدون شروع شیفت: هشدار بدون پاک کردن فرم.
- بارکد قبض ≠ کد ملی.

### Changed — طراح چاپ
- بدون انتخاب بخش؛ طرح برای چاپگر همین دستگاه است.
- تنظیمات چاپ: «طرح‌های تعریف‌شده» با پیش‌فرض غیرقابل‌حذف.
- ابزار انتخاب (rubber-band) و دست (pan). قالب‌ها پر و ریسپانسیو. بیمه پایه بدون درصد.

### Fixed — تم نئون
- صورت‌حساب، مبلغ نهایی، جدول صندوق، جستجوی قبض، صف پذیرش.

---

## 1.98.0 — 2026-08-26 — ترتیب حساب‌ها + قالب‌های متمایز + بارکد واقعی + قیمت خدمات

### Changed — صفحه ورود
- RTL از راست: حسابداری | حساب پرسنل | حساب مدیریت.

### Changed — ۳۰ قالب چاپ پذیرش
- همان لیبل‌ها و فیلدها؛ چیدمان هر خانواده فرق دارد (خدمات بالا/پایین، دو ستونه، ستون کناری، ته‌برگ، مالی اول). A4 و ریسپانسیو. مهاجرت tpl_migration_1_98.

### Fixed — صندوق و قبض
- فونت وزیر روی لیست انتظار / استرداد / بدهکار / بستانکار؛ جدول ارتفاع کامل + اسکرول.
- بارکد واقعی Code128 جدا از کد ملی و شماره قبض.
- پذیرش اصلی: مینی پرداخت (پرداخت/نقدی/رایگان/تست) نه مینی بارکد. مینی بارکد فقط زیربخش.
- تست پرداخت واقعاً مبلغ را صندوق می‌کند.
- پرداخت‌شده‌ها از صندوق خارج و در جستجوی قبض می‌مانند. به‌روزرسانی زنده.

### Fixed — پذیرش بیمار
- پزشک معالج لیست‌باکس مثل انجام‌دهنده.
- قیمت آزاد/دولتی/بیمه + ضریب در صورت‌حساب.
- حذف لیبل «اطلاعات بیمار کامل و آماده پذیرش است».

---

## 1.97.0 — 2026-08-26 — صندوق/قبض/پوز + کارتابل واتساپی + حسابداری + قالب‌های پذیرش

### Added — صندوق در برابر قبض + دستگاه پوز
- تیک «بخش دستگاه پوز دارد» / «زیر بخش دستگاه پوز دارد» در تعریف بخش (پیش‌فرض خاموش).
- بدون پوز: قبض وارد صندوق می‌شود و مینی‌صفحه بارکد/خدمات/چاپ باز می‌شود.
- با پوز: خودشان تسویه می‌کنند و در صندوق نمی‌آیند.
- نوار جدا: قبوض استردادی / ایست انتظار / بدهکار / بستانکار.
- مینی پرداخت: کارتخوان، نقد، رایگان، تخفیف، تست.
- دابل‌کلیک قبض → فرم قفل؛ F5 باز می‌کند؛ چاپ همان قبض اصلی. دکمه چاپ بدون باز کردن فرم.

### Added — کارتابل و حسابداری
- اعلان پایین‌راست هنگام آنلاین شدن: «شما N پیام خوانده‌نشده دارید».
- کارتابل واتساپی: اولویت رنگی، چت دوطرفه، تأیید ارسال، راست‌کلیک سنجاق/ذخیره/چاپ/حذف.
- دکمه حسابداری در صفحه ورود (آیکون کیف، وسط‌چین با پرسنل و مدیریت).

### Fixed — تم نئونی/تیره
- شمارنده پرداختی/صندوق/ثبت‌شده/در انتظار، آیکن باز/بسته خدمات، افزودن خدمت.
- صفحات مدیریت CRM و ادمین با تم تیره/نئونی همگام می‌شوند.

### Changed — طراح چاپ
- ۳۰ قالب مخصوص پذیرش؛ جدول خدمات: نام خدمت | تعداد | مبلغ کل | شرح خدمت (بدون مثال).
- بارکد استاندارد زیر جدول. سن `10Y`. Ctrl+P = چسباندن. ریسپانسیو A4→حرارتی حفظ شد.

---

## 1.96.0 — 2026-08-26 — رفع کامل تم نئونی/تیره + گسترش طراح چاپ + ریسپانسیو قالب‌ها

### Fixed — تم نئونی و تیره (کامل)
- تمام تداخل‌های ظاهری رفع شد: همه بخش‌ها در تم نئونی/تیره تیره با متن سفید.
- نئونی: نورپردازی و glassmorphism با glow نئونی (cyan/green/pink).
- هیچ بک‌گراند سفیدی در تم نئونی/تیره.

### Changed — طراح چاپ و قالب‌ها
- ۳۰ قالب پرینت پزشکی گسترش یافت: آیتم‌های بیشتر (۲۵-۴۰ در هر قالب).
- ریسپانسیو: قالب‌ها هنگام تغییر سایز (A4→رول حرارتی) سایزش می‌شوند.
- بارکد استاندارد (۶-۸mm ارتفاع، نه بزرگ).
- لیبل‌ها PIT_LABEL، مقادیر PIT_FIELD با فیلد بایندینگ ({full}, {age}, ...).
- جدول خدمات: شرح خدمت | # | نام خدمت | تعداد | مبلغ کل.
- میان‌برهای کیبورد: Ctrl+S/Z/Y/C/P در طراح.
- تنظیمات گسترده: سایز کاغذ، حاشیه، فونت، بارکد.
- فیلدهای بیشتر: پذیرشگر، صندوق، ش.ص، نسخه الکترونیک.

---

## 1.95.0 — 2026-08-26 — رفع کامل تم نئونی/تیره + ماشین حساب LTR + پرینت پزشکی + صندوق + بهینه‌سازی

### Fixed — تم نئونی و تیره (کامل)
- تمام تداخل‌های ظاهری رفع شد: بک‌گراند پذیرش، هدرها، دکمه‌ها، ابزارها — در تم تیره/نئونی تیره با متن سفید.
- متن ابزارها و هدرها در تم تیره/نئونی سفید.
- دکمه بازگشت تیره با متن سفید.
- نئونی: نورپردازی و glassmorphism با glow.

### Fixed — ماشین حساب
- گرید به LTR (مثل numpad) تغییر کرد. اعداد فارسی با ترتیب درست.

### Changed — بهینه‌سازی باز شدن تب
### Changed — صندوق
- درآمد شیفت در همان روز جمع می‌شود. فونت Vazir.
### Changed — لیست سیاه / کارتابل
- ظاهر بهتر، فونت Vazir.
### Added — پرینت پزشکی
- ۳۰ مدل طراحی پرینت قبض پزشک به‌روزرسانی شد.

---

## 1.94.0 — 2026-08-26 — رفع باگ‌های تم نئونی/تیره + کارتابل پیام حرفه‌ای + لیست سیاه HTML + پزشک معالج + F5

### Fixed — تم نئونی و تیره
- تداخل ظاهری در تم نئونی رفع شد: بک‌گراند پذیرش، صورت‌حساب، خدمات، عملیات، پروفایل بیمار، دکمه‌های چاپ — همه در تم نئونی تیره با نورپردازی نئونی.
- تداخل ظاهری در تم تیره رفع شد: عملیات پذیرش، ماندہ قابل پرداخت، پروفایل بیمار، چاپ اسناد — همه در تم تیره تیره با متن سفید.
- متن هدرها (صورت‌حساب و...) در تم تیره/نئونی سفید می‌شوند.
- اعداد سهم بیمار در تم نئونی قابل مشاهده هستند.

### Fixed — ماشین حساب
- باگ ترتیب اعداد برعکس رفع شد: اعداد حالا از چپ به راست (LTR) نمایش داده می‌شوند (۱۲۳ نه ۳۲۱).

### Fixed — ظاهر صفحه اول و هدر
- بک‌گراند ظرف وسطی صفحه اول سفید با سایه ظریف شد.
- هدر سفید با سایه ظریف و پالت رنگی شد (نه یک‌دست سفید).

### Changed — کارتابل پیام (HTML)
- ظاهر کارتابل پیام مثل پیام‌رسان‌های موبایل بازطراحی شد: تایل‌های پیام با border و سایه، نمای جزئیات با هدر و پروفایل فرستنده.
- سرچ باکس کوتاه‌تر و وسط هدر.
- منوی همبرگر از سمت راست باز می‌شود و با کلیک بیرون بسته می‌شود.
- پیام‌های سنجاق‌شده بالای لیست قرار می‌گیرند.

### Added — لیست سیاه بیماران HTML
- لیست سیاه از تنظیمات C++ به داشبورد HTML منتقل شد.
- در اقدامات اصلی داشبورد: پذیرش بیمار / تب جدید / لیست سیاه بیماران.
- فرم ثبت مسدودی، جستجو، تاریخچه، رفع مسدودی.

### Changed — پزشک معالج
- سرچ پزشک معالج حالا مثل انجام‌دهنده auto-select می‌شود: سرچ کردیم سریع انتخاب می‌کند.

### Added — کلید F5
- F5 مجوز ویرایش اطلاعات پذیرش بیمار: وقتی قبض با دابل‌کلیک باز می‌شود، فرم قفل است؛ F5 ویرایش را فعال می‌کند.

### Changed — صندوق
- سرچ باکس صندوق کوتاه‌تر و وسط شد.

---

## 1.93.0 — 2026-08-25 — Soft Medical Glassmorphism + تم نئونی + کارتابل پیام HTML + صندوق + تنظیمات

### Added — تم نئونی (Neon)
- تم سوم اضافه شد: **نئونی** — ترکیبی از Cyberpunk + Retro Futurism: بک‌گراند آبی-مشکی عمیق، accent فیروزه‌ای نئونی `#00F0FF`، سبز نئونی `#00FF88`، صورتی نئونی `#FF0066`.
- تم نئونی روی همه سطح‌ها (C++ نیتیو + HTML) اعمال می‌شود.
- دکمه انتخاب تم نئونی در تنظیمات اضافه شد.

### Added — کارتابل پیام HTML
- کارتابل پیام از C++ نیتیو به **HTML** تبدیل شد با ظاهر حرفه‌ای.
- منوی همبرگری، جستجو، آرشیو، سنجاق (pin)، چاپ، ذخیره، پاسخ.
- پیام‌های ذخیره‌شده از تنظیمات C++ به منوی همبرگری کارتابل منتقل شد.
- اعلان پیام جدید در کل برنامه sync شده است.

### Added — فلگ «ثبت پذیرش در صندوق»
- در تعریف بخش/زیربخش تیک «ثبت پذیرش در صندوق» اضافه شد (دیفالت خاموش).
- اگه روشن شود، بخش به سر تب‌های صندوق اضافه می‌شود.

### Changed — پالت «Soft Medical Glassmorphism»
- پالت روشن جدید: Ice White `#E8EEF5`، شیشه نیمه‌شفاف `#F2F7FC`، آبی پزشکی `#2D5FE6`، متن سرمه‌ای `#0D1B2A`.
- gradient بسیار ملایم سفید→آبی یخی، سایه نرم پخش‌شده، border سفید نیمه‌شفاف + خط آبی ظریف.

### Changed — تنظیمات
- تنظیمات حالا **fullscreen** است (کل صفحه شامل taskbar).
- «تنظیمات پذیرش» از گزینه‌های تنظیمات حذف شد.
- «پیام‌های ذخیره‌شده» از تنظیمات C++ حذف و به کارتابل پیام HTML منتقل شد.

### Changed — پذیرش بیمار
- باگ آیکن صورت حساب سفید رفع شد.
- خط رنگی بالای عملیات پذیرش به سبز (success) تغییر کرد.
- خط رنگی بالای چاپ به slate تغییر کرد.
- کارت‌های وسط (بیمه/پزشک/انجام‌دهنده) کلاس‌های CSS درست گرفتند و ظاهرشان بهتر شد.
- بهینه‌سازی لود: performers و queue در init response ادغام شدند (حذف 2 round-trip).

### Changed — صندوق
- دکمه‌های شروع/پایان شیفت حالا با opacity کم و grayscale غیرفعال می‌شوند.
- تاریخ و ساعت شیفت به دو خط (تاریخ بالا، ساعت پایین) و در ردیف دکمه‌ها منتقل شد.
- تب‌های صندوق با glassmorphism و border و رنگ‌بندی زیبا استایل شدند.
- پوشش تم تیره برای کارت‌های خلاصه صندوق اضافه شد.

### Changed — ماشین حساب
- فلش بصری هنگام فشردن کلید کیبورد: دکمه متناظر تغییر رنگ می‌دهد و سریع برمی‌گردد.

### Changed — تم تیره روی HTML
- تم تیره روی همه سطح‌ها (admission/tools/cashier/queue/dash/portal) اعمال می‌شود.
- متن و لیبل‌ها در تم تیره سفید هستند — تداخل ظاهری نیست.

---

## 1.92.0 — 2026-08-25 — بازطراحی کامل UI + پالت استاندارد + لاگ خطاهای HTML + ساختار ایمن

### Added — لاگ خطاهای HTML
- پوشهٔ **`logs/html errors`** ساخته می‌شود و فقط **خطاها و کرش‌ها و باگ‌ها** را ثبت می‌کند: خطاهای JS (`window.onerror`)، خطاهای caught، و شکست لود DOM. **هیچ لاگ معمولی** (لاگین، باز شدن صفحه) ثبت نمی‌شود (`src/util.cpp::logHtmlError`, `src/web_admission_api.inc::client.error`, `admission.js`, `bridge.js`).

### Added — مستندات ساختار و دیزاین
- **`docs/DESIGN_SYSTEM.md`**: پالت رنگی استاندارد (تم روشن/تیره)، قوانین طراحی (D-1…D-12)، استایل Concave، قوانین کامپوننت، و قانون همگام‌سازی فایل-تم. هر مدلی که روی برنامه کار می‌کند باید این قوانین را رعایت کند.
- **`docs/ARCHITECTURE.md`**: نقشهٔ فایل‌ها، معماری سطح‌های تعبیه‌شده، و **۸ قانون ایمنی بحرانی** برای مدل‌های هوش مصنوعی (عدم حذف عناصر صفحات دیگر، CSS نام‌اسپیس‌شده، verbهای append-only، RCDATA ثابت، -Werror). ساختار طوری تعریف شده که آپدیت‌های سنگین روی یک صفحه، صفحات دیگر را پاک نکنند.

### Fixed — باگ داشبورد پیش‌فرض
- وقتی WebView2/MSHTML در دسترس نباشد، داشبورد دیگر به فرم خالی پذیرش fallback نمی‌کند — ریشهٔ «یهو می‌رود توی پذیرش» مرتب شد (`src/reception.cpp`).

### Changed — پالت رنگی استاندارد (v1.92.0 «Medical Glass»)
- **تم روشن**: surface از خالص سفید `#FFFFFF` به آف‌وایت `#F1F4F9` تغییر کرد (عمق شیشه‌ای)؛ bg عمیق‌تر `#C3CDDD`؛ border حالا کاملاً مرئی `#A8B5CC`؛ متن پر‌کنتراست `#1A2435`؛ accent آبی سلطنتی `#2B48CC` (`src/theme.cpp::applyTheme`).
- **تم تیره**: bg `#0B0F15`، surface `#151B25`، border مرئی `#2C3645`، accent آبی زنده `#3B82F6`.
- CSS تعبیه‌شده (`admission.css`) کاملاً با پالت جدید همگام شد.

### Changed — صفحهٔ اول (انتخاب حساب)
- ظرف داخلی آیکون‌ها به عرض ظرف بیرونی نزدیک شد (`src/main.cpp::homeGeom`).
- **گلو (halo) رنگی** از روی آیکون‌های حساب حذف شد؛ سایهٔ خنثی ظریف جایگزین شد (`paintAppIcon`).
- بک‌گراند روشن‌تر و تمیزتر (scrim آلفا کاهش یافت).

### Changed — هدر/فوتر C++
- فونت ساعت بزرگ‌تر شد (`g_fClock` 22px bold) و فونت تاریخ (`g_fDate` 14px)؛ ارتفاع هدر `S(60)`.
- رنگ‌بندی هدر روشن‌تر و خوانا (`src/main.cpp`).

### Changed — تب‌های C++
- عرض ایده‌آل کمتر (`S(188)`)، حداقل عرض `S(84)`، فاصله `S(6)` → تب‌های بیشتری جا می‌گیرند.
- دکمهٔ بستن **کارتابل پیام** حالا همیشه قابل دسترس است؛ badge پیام خوانده‌نشده به منطقهٔ عنوان منتقل شد تا × هرگز پوشانده نشود (`src/reception.cpp`).

### Changed — صفحهٔ پذیرش بیمار (CSS)
- بنفش از کارت پزشک معالج حذف شد → آبی accent؛ سبز کارت بیمه → سبز موفقیت؛ فیروزه‌ای کارت انجام‌دهنده → آبی accent. رنگ‌ها حالا هماهنگ و استاندارد هستند (`admission.css`).
- صورت‌حساب: هدر کهربایی (amber) `#D97706`، مانده قابل پرداخت کهربایی؛ مبلغ نهایی emerald **Bold 30px** و کاملاً مشخص.
- لیبل‌ها استاندارد شدند (`#2E3D52`، 12px)؛ خط‌های جداکننده زیر قسمت‌ها (نه خط کامل بالا).
- سرویس‌ها: border ریسپانسیو با فاصلهٔ کم از گوشه‌ها.

### Changed — صفحهٔ ابزارها
- پوستهٔ کرم/بژ حذف شد؛ با پالت استاندارد همگام شد؛ بک‌گراند متمایز از پذیرش (`admission.css`).

### Changed — صندوق
- دکمه‌های شروع/پایان شیفت حالا **enable/disable** می‌شوند طبق وضعیت شیفت (`admission.js::renderCash`).
- زبانهٔ «صندوق نرفته‌ها» از هدر به **بالای جدول** منتقل شد (`#cashTabsAbove`).
- نمایش تاریخ/ساعت شیفت زیباتر شد (گرادیان، border sólido، 13px).

### Files Changed
- `src/theme.cpp`, `src/main.cpp`, `src/reception.cpp`, `src/util.cpp`, `src/app.h`, `src/app.rc`
- `assets/admission/admission.css`, `assets/admission/index.html`, `assets/admission/admission.js`, `assets/admission/bridge.js`
- `src/web_admission_api.inc`
- `docs/DESIGN_SYSTEM.md` (new), `docs/ARCHITECTURE.md` (new), `docs/CHANGELOG.md`
- `scripts/test_admission_assets.py`
- `update/version.txt`, `build/DarmanPlus.exe`

---

## 1.90.0 — 2026-08-25 — بازطراحی UI + بستن کارتابل + تب‌های منعطف

### Fixed
- تب **کارتابل پیام** حالا دکمهٔ بستن دارد؛ قبلاً حتی بدون پیام خوانده‌نشده هم `continue` می‌شد و × هرگز رسم نمی‌شد (`src/reception.cpp`).
- تب‌های C++ دیگر از صفحه بیرون نمی‌زنند: عرض پویا تا `S(118)`، و اگر هنوز جا نباشد فلش اسکرول + چرخ موس (`src/reception.cpp`).
- باز شدن داشبورد و ابزارها دیگر فرم پذیرش را پر نمی‌کند (select/خدمات/صف/zoom)، پس گیر و تأخیر کمتر است (`admission.js`).

### Changed — صفحهٔ ورود (نیتیو)
- ظرف شیشه‌ای آیکون‌ها هم‌عرض پنل پشت سر (inset `S(28)`) با خط جداکنندهٔ نازک بین دو حساب؛ پنل و سینی عمیق‌تر و غیرسفید؛ نشان حساب کمی بزرگ‌تر با صفحهٔ رنگی زیر نشان (`src/main.cpp`).
- ساعت هدر با جوهر `sectionInk` و تاریخ با `labelInk` — خوانا روی نوار شیشه‌ای؛ فوتر هم پررنگ‌تر (`src/main.cpp`, `src/theme.cpp`).

### Changed — داشبورد پرسنل
- پالت Aurora (بوم `#A9BDD8`، هدر شیشه‌ای، ظرف `#E4EEF8`)؛ جستجو کوتاه، وسط‌چین، مقعر با گوشهٔ گرد.
- **کارتابل پیام** از «اقدامات اصلی» حذف شد (دکمه‌اش در هدر می‌ماند). **پذیرش بیمار** راست، **تب جدید** چپ.

### Changed — ابزارها
- پالت Workshop Clay (بوم گرم `#C9BBA8`) تا شبیه داشبورد نباشد.
- حذف زیرعنوان «همه ابزارهای پذیرش…» و توضیح‌های کمکی دراور/جستجوی قبض.

### Changed — پذیرش بیمار
- بوم `#C2D0E4`، کارت‌های رنگی (نه سفید)، فیلدهای Concave، هدر بخش‌ها با نوار ۳px بالا + نیم‌خط پایین، برچسب‌های پرکنتراست، **مبلغ نهایی** درشت‌تر. محاسبات قبض دست‌نخورده است.

---

## 1.89.0 — 2026-08-24 — داشبورد پرسنل + اعلان پیام + بهینه‌سازی FPS صفحهٔ ورود

### Added — داشبورد حساب پذیرش (تب نیتیو دائمی)
- بعد از ورود به حساب پذیرش، **داشبورد** به‌عنوان تب اول و دائمی باز می‌شود (قابل بستن نیست)؛ نوار اکشن هدر («پذیرش بیمار»/«تب جدید») کاملاً حذف شد و آن اقدامات حالا اپ-آیکون‌های داشبوردند.
- طراحی داشبورد: بوم تیره‌تر با بافت، ظرف شیشه‌ای (Glassmorphism)، نوار بالایی شیشه‌ای با همبرگر + **آیکون پاکت‌نامهٔ کارتابل با بج عدد قرمز** (صفر = خاکستری)، سرچ‌باکس Concave، سه اپ-آیکون به سبک اندروید (کارتابل پیام، تب جدید، پذیرش بیمار — بدون کارت)، دراور دسته‌بندی‌شده با جستجو.
- تب «کارتابل» حالا قابل بستن است؛ بج قرمز خوانده‌نشدگان روی تب مثل قبل، اما با پاک شدن بعد از مشاهده.
- **اعلان Toast پایین-راست** برای پیام جدید: حیاتی=قرمز، فوری=کهربایی، عادی=آبی؛ کلیک چپ → باز کردن کارتابل بدون از دست رفتن حالت تب فعلی؛ کلیک راست → منوی «محو»؛ بستن خودکار بعد از ۹ ثانیه.

### Fixed — صفحهٔ ورود
- **رفع لگ/Drop FPS**: انیمیشن shimmer حذف شد؛ نقاشی صفحه به dirty-rect تبدیل شد (هاور اپ-آیکون فقط همان سلول را بازنقاشی می‌کند)؛ روبان Gradialism حالا یک گرادیان سه‌ایستگاهی واقعی بدون درز وسط است (`gpGradRibbon3` با SetInterpolationColors).
- توضیحات زیر آیکون‌های حساب حذف شد — فقط آیکون + نام؛ صفحه عمیق‌تر و کمتر سفید.

### Changed — پذیرش و ابزارها
- صورتحساب: شیشهٔ **زرد بسیار کم‌رنگ** (نه bold)؛ «مانده قابل پرداخت» فقط کمی پررنگ‌تر با border در همان خانوادهٔ رنگی؛ قهرمان «مبلغ نهایی» دیگر دیوار زمردی نیست — شیشهٔ ساکت با حاشیه و مرکزیت زمردی.
- هدر کارت‌ها (مشخصات بیمار / اطلاعات بیمه / پزشک معالج / انجام دهنده) با گرادیان دولایهٔ عمیق‌تر و چیپ آیکون شیشه‌ای.
- ابزارها: هاور کاشی دیگر بک‌گراند نمی‌گیرد — فقط آیکون کمی بزرگ‌تر و نام پررنگ‌تر می‌شود (سبک iOS)؛ گرید داخل ظرف شیشه‌ای؛ توضیحات و دکمهٔ «باز کردن» حذف شدند.
- راهنمای رنگ جستجوی قبض: هر ردیف = دایرهٔ رنگ خالص + چیپ متن خوانا با همان رنگ پس‌زمینه.
- تب‌های C++: حالت عادی/هاور شیشه‌ای با لبهٔ نور داخلی.
- **انتقال برنامه**: اگر exe جابه‌جا شود، آماده‌سازی سیستم (فونت، پیکربندی) دوباره پیش از اجرا انجام می‌شود (`setup_exe_path`).

---

## 1.88.0 — 2026-08-24 — رفع باگ خط مشکی هدر + دکمه‌های App-Icon و پالت آبی سلطنتی

### Fixed
- **خط مشکی ۱ پیکسلی وسط هدر**: بافت بافر دوبلهٔ فریم مقداردهی‌نشده (سیاه) بود و `gpGradRoundRect` یک ردیف پیکسل آخر را نمی‌پوشاند؛ حالا کل نوار ابتدا با رنگ پایه پر می‌شود (`src/main.cpp` frameProc).
- برش عنوان صفحهٔ خوش‌آمد و حذف سه‌تکهٔ چیپ‌ها بعد از کوچک‌شدن کارت‌ها: حداقل عرض پنل `S(760)` شد.
- کلیک روی آیکون‌های حساب دیگر به حالت hover وابسته نیست — hit-test مستقیم از مختصات (لمس/کلیک سریع هم کار می‌کند).
- دکمه‌های primary/submit/outline صفحات HTML: قانون قدیمی v1.78 با specificity بالاتر سبک Concave را خفه می‌کرد؛ با سلکتور قوی‌تر دوباره اعمال شد.

### Changed — صفحهٔ انتخاب حساب (نیتیو)
- دکمه‌ها به **اپ-آیکون‌های iOS** تبدیل شدند: بدون هیچ پس‌زمینه/قاب، نشان squircle براق با سایهٔ رنگی + نام حساب زیرش؛ داخل یک **ظرف شیشه‌ای** (tray) تازه. چون مستقیم روی بافر پنل نقاشی می‌شوند، باگ «گوشه‌های تیز با بک‌گراند باقی‌مانده» به‌طور ساختاری منتفی است.
- پالت اصلی به آبی سلطنتی عمیق‌تر (#3550D8) تغییر کرد؛ دکمه‌های Concave حالا پایین کم‌نور‌تر دارند تا متن سفید همیشه خوانا بماند.
- برچسب دکمه‌های C++ با **وزیرمتن Bold**؛ ساعت هدر با فونت ضخیم آبی-مرکزی + تاریخ accent و خط‌چین‌های الماسی.
- هاله/ریبون/انیمیشن ورود v1.87 حفظ شد.

### Changed — HTML
- **ابزارها**: کاشی‌ها کاملاً بی‌پس‌زمینه (آیکون + نام + توضیح + «باز کردن»)، حذف نشان F7، بوم دوریختی دو-رنگ با بافت مورب، نوار بالایی شیشه‌ای، دراور دسته‌بندی با آیتم‌های لبه-accent شیشه‌ای.
- **جستجوی قبض**: حالا **تب نیتیو جداگانهٔ C++** است (باز شدن از ابزارها → تب تازه؛ «بازگشت به ابزارها» تب را می‌بندد و به تب ابزارها برمی‌گردد — حالت ابزارها دست‌نخورده می‌ماند). راهنمای رنگ ردیف‌ها از ریل فیلتر به **دایرهٔ طیفی در هدر** منتقل شد (hover → پاپ‌اور شیشه‌ای). دکمهٔ جستجو با ارتفاع درست زیر فیلترها.
- **صندوق**: کارت‌های آماری با هویت پاستلی مجزا (آبی/سبز/کهربایی/بنفش)، «شیفت شروع نشده» به قرص کوچک وسط‌چین، دکمه‌های شیفت باریک‌تر و قرصی.
- **پذیرش**: کارت پروفایل شیشهٔ آبی عمیق‌تر با حلقهٔ آواتار برند، صورتحساب از کهربایی به آبی سلطنتی شیشه‌ای (گروه‌ها خنک، «مانده» رز ظریف)، دکمهٔ ابزارها سبز-آبی تیره و واضح.
- هم‌رنگی hue صفحات HTML با accent جدید C++ (هدر صف، آیکون چیپ‌ها، لانچر صف).
...
## 1.85.0 — 2026-08-24 — رفع کرش زبانه ابزارها، صف به زبانهٔ نیتیو، تم شیشه‌ای/ Clay

### Fixed — کرش باز کردن ابزارها / صندوق
- باز کردن یا بستن زبانهٔ ابزارها/صندوق/صف دیگر داخل همان پاسخِ WebView2 انجام نمی‌شود؛ ساخت/تخریب WebView2 به یک نوبت تازهٔ حلقهٔ پیام منتقل شد (`RunOnUiThread`)، پس کرش `0xC0000005` هنگام باز کردن ابزارها برطرف شد.

### Changed — صف پذیرش زبانهٔ نیتیو C++
- «صندوق نرفته‌ها و صف پذیرش» در زبانهٔ جدید C++ باز می‌شود (نه روکش HTML). بازخوانی/دابل‌کلیک یک بیمار، قبض پرداخت‌نشدهٔ واقعی را با مبلغ و خدمات در زبانهٔ پذیرش تازه باز می‌کند.

### Changed — تم Claymorphism × Liquid Glass
- صفحهٔ انتخاب حساب، هدر، دکمه‌های پذیرش و سایر سطوح نیتیو: پالت ملایم‌تر و accent یکدستِ نیلی‌آبی، کارت‌های گِلی با سایهٔ نرم عمیق‌تر و گوشه‌های گردتر.
- صفحه‌های پذیرش/ابزارها/صندوق/صف: کارت‌ها و کاشی‌های شیشه‌ای، ورودی‌های فرورفتهٔ نئومورفیک، دکمه‌های گِلی، هدرهای گرادیانی نرم.
- صندوق: متن «درآمد شیفت» حذف شد — فقط عدد با جداکنندهٔ سه‌رقمی فارسی + «ریال» در یک قرص شیشه‌ای برجسته در سمت چپ هدر.
- رنگ دکمهٔ ابزارها از آبی تند به سبز-آبی زمردی تغییر کرد.
- آیکون‌های تکراری پذیرش (مشخصات بیمار / پزشک معالج) یکتا شدند.

### Files
- پذیرش: `assets/admission/index.html`, `admission.js`, `admission.css`
- هسته: `src/app.h`, `src/app.rc`, `src/reception.cpp`, `src/theme.cpp`, `src/main.cpp`, `src/web_admission_embed.inc`, `src/web_admission_api.inc`, `src/web_crm_api.inc`, `update/version.txt`

---

## 1.84.0 — 2026-08-23 — زبانه نیتیو ابزارها و صندوق، زوم پایدار

### Fixed — زوم پذیرش
- آخرین اندازهٔ دیده‌شده بعد از خروج/ورود می‌ماند؛ اگر اندازهٔ ذخیره‌شده سه ستون را بشکند، پله‌پله کم می‌شود یا به ۸۰ برمی‌گردد و همان مقدار امن ذخیره می‌شود.
- «ریست کردن بزرگنمایی» در تنظیمات نیتیو هدر اضافه شد.
- ذخیرهٔ حالت نمایش ساده/کامل دیگر زوم امن را بازنویسی نمی‌کند (بازه ۵۰–۲۰۰، پیش‌فرض ۸۰).

### Changed — زبانه‌های نیتیو
- ابزارها، صندوق و بازخوانی قبض به‌صورت زبانهٔ C++ باز می‌شوند؛ نوار تب HTML حذف شد.
- دکمهٔ ابزارها بعد از «صندوق نرفته‌ها و صف پذیرش» و قبل از چاپ اسناد است.
- F7 یک زبانهٔ «صندوق» باز می‌کند، نه روکش روی همهٔ صفحات.
- دابل‌کلیک قبض زبانهٔ پذیرش تازه باز می‌کند و فرم را اول خالی می‌کند.

### Changed — ظاهر پذیرش، صف، صندوق و جستجوی قبض
- ریل پذیرش اداری‌تر است؛ صف از زوم پذیرش مستقل است و دکمهٔ مردهٔ پایین روکش حذف شد.
- آیکون‌های ابزارها شیشه‌ای‌اند. درآمد شیفت سمت چپ هدر صندوق است؛ شروع/پایان شیفت و سند دستی بالای جدول‌اند؛ جستجو زیر هدر است.
- کشیدن عرض ستون فقط همان ستون را عوض می‌کند.
- راهنمای رنگ جستجوی قبض بالای جدول است؛ صفحه‌بندی ۳۰ ردیفی با «صفحه N از M»؛ خروجی اکسل صفحهٔ جاری است.

### Files
- پذیرش: `assets/admission/index.html`, `admission.js`, `admission.css`
- هسته: `src/app.h`, `src/app.rc`, `src/reception.cpp`, `src/settings.cpp`, `src/user_settings.cpp`, `src/main.cpp`, `src/web_admission.h`, `src/web_admission_embed.inc`, `src/web_admission_host.inc`, `src/web_admission_mshtml.inc`, `src/web_admission_dispatch.inc`, `src/web_admission_api.inc`, `src/web_crm_api.inc`, `update/version.txt`
