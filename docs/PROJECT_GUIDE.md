...
> **نسخهٔ فعلی: ۱.۹۱.۰** — بازطراحی کامل UI روی «سیستم طراحی» واحد (`docs/DESIGN_SYSTEM.md`)، ساختار لایه‌بندی ضدخرابکاری با قرارداد اجرایی (`AGENTS.md`, `scripts/check_ui_contract.py`)، و بازسازی صندوق بر پایهٔ بخش‌ها:
...
> - **یک سیستم طراحی واحد** (`docs/DESIGN_SYSTEM.md`) برای هم صفحه‌های نیتیو و هم صفحه‌های HTML: پالت مشترک، قانون «یک رنگ برای هر پنل»، نیم‌خط کوتاه زیر عنوان بخش‌ها، و فیلدهای تخت با حاشیهٔ واضح. حالت inset/concave و رنگ‌های بنفش/صورتی/کِرِم ممنوع شده‌اند.
> - **دو تم**: روشن (پیش‌فرض) و تیره. تم‌های «آرام» و «گرم» در نسخهٔ ۱.۷۷ بازنشسته شدند و C++ دیگر آن‌ها را نمی‌فرستد.
> - **قرارداد UI اجرایی**: `build.sh` پیش از کامپایل `scripts/check_ui_contract.py` را اجرا می‌کند تا آپدیت‌های بعدی نتوانند بی‌صدا آیتم‌ها یا استایل صفحه‌های دیگر را حذف کنند.
